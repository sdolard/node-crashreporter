...
crashReporterSingleton  = (function() {
	...
	function checkHiddenAttribute(name) {
	...
	CrashReporter.prototype.onUncaughtException = function(err) {
		...
		content = [],
		data;
		if (!checkHiddenAttribute('dateTime')) content.push('Date time: ' + now);
		if (!checkHiddenAttribute('execPath')) content.push('execPath: ' + process.execPath);
		if (!checkHiddenAttribute('argv')) content.push('argv: ' + process.argv.join(', '));
		if (!checkHiddenAttribute('currentDirectory')) content.push('current directory: ' + process.cwd());
		if (!checkHiddenAttribute('env')) content.push('env:\n' + util.inspect(process.env));
		if (process.platform !== 'win32') {
			if (!checkHiddenAttribute('gid')) content.push('gid: ' + process.getgid());
			if (!checkHiddenAttribute('uid')) content.push('uid: ' + process.getuid());
		}
		if (!checkHiddenAttribute('pid')) content.push('pid: ' + process.pid);
		if (!checkHiddenAttribute('processTitle')) content.push('process title: ' + process.title);
		if (!checkHiddenAttribute('uptime')) content.push('uptime (sec): ' + process.uptime());
		if (!checkHiddenAttribute('arch')) content.push('arch: ' + process.arch);
		if (!checkHiddenAttribute('versions')) content.push('versions:\n' + util.inspect(process.versions));
		if (!checkHiddenAttribute('memoryUsage')) content.push('memory usage:\n' + util.inspect(process.memoryUsage()));
		if (!checkHiddenAttribute('requireCache')) content.push('require.cache:\n' + Object.keys(require.cache).join('\n'));
		if (!checkHiddenAttribute('activeHandle')) content.push('active handle:\n' + util.inspect(process._getActiveHandles()));
		if (!checkHiddenAttribute('activeRequest')) content.push('active request:\n' + util.inspect(process._getActiveRequests()));
		if (!checkHiddenAttribute('error')) content.push(this.error);
